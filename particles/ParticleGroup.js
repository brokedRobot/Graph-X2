...
//
// ParticleGroup may be freely distributed under the MIT license (See LICENSE.txt)
...
function ParticleGroup( options ) {
    ...
    that.scale               = parseInt( options.scale || 1 );
    ...
    that.position              = options.position || new THREE.Vector3(0, 0, 0);
    ...
    that.attributes = {
        ...
        age:            { type: 'f', value: [] },
        size:           { type: 'f', value: [] },
        //ownscale:           { type: 'f', value: [] },
        //rotation:           { type: 'f', value: [] },
        originalsize:   { type: 'f', value: [] },
        sizeEnd:        { type: 'f', value: [] },
        originalsizeEnd:        { type: 'f', value: [] },
	...
	that.mesh.position = that.position;
...
ParticleGroup.prototype = {
     ...
     * updated when ParticleGroup#tick() is called.
     * 
    ...
    addEmitter: function( emitter ) {
        ...
        var vertices = that.geometry.vertices,
            ...
            size = a.size.value,
            //scale = a.scale.value,
            originalsize = a.originalsize.value,
            sizeEnd = a.sizeEnd.value,
			originalsizeEnd = a.originalsizeEnd.value,
        ...
        for( var i = start; i < end; ++i ) {
            ...
            originalsize[i] = that._randomFloat( emitter.size, emitter.sizeSpread );
            //scale[i] = that._randomFloat( emitter.size, emitter.sizeSpread );
			//here set another attribute/uniform to size[i] that doesnt change until particle reset
			//emitter.sizes[i]   = size[i];
            sizeEnd[i]   = originalsizeEnd[i]    = emitter.sizeEnd;
            //originalsizeEnd[i]      = emitter.sizeEnd;
            age[i]          = 0.0;
    ...
    getFromPool: function() {
        ...
        if( pool.length ) {
            ...
            return new ParticleEmitter( that._poolCreationSettings );
        }
    ...
    addPool: function( numEmitters, emitterSettings, createNew ) {
        ...
        for( var i = 0; i < numEmitters; ++i ) {
            emitter = new ParticleEmitter( emitterSettings );
...
ParticleGroup.shaders = {
    vertex: [
        ...
        'vec4 GetPos() {',
            ...
            'vec4 mvPosition = modelViewMatrix * vec4( newPos.x, newPos.y, newPos.z, 1.0 );',
        ...
        'void main() {',
            ...
            'if( alive > 0.5 ) {',
                ...
                //'vec4 posb = projectionMatrix * pos;',
                //'gl_Position = vec4(posb.x,posb.y,posb.z,1.0);',
