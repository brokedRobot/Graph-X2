	...
	(function(){
		...
		*/
		Distorter.prototype.getAppliedOffset = function(y, t, distortEffect, ampl, ampl_accel, s_freq, s_freq_accel, compr, compr_accel, speed) {
			...
			//console.log((ampl + ampl_accel * t * 2));
			var amplitude = (ampl + ampl_accel * t * 2);//-(Math.random()*2048);
			var frequency = (s_freq + s_freq_accel * t * 2);//-(Math.random()*2048);
			...
			if (distortEffect == 1){
				...
				return L;///((parseInt(Math.random()*2)+1)*64);
			}
		...
		Distorter.prototype.ComputeFrame = function(
			dst, 
			src, 
			distortEffect, 
			letterbox, 
